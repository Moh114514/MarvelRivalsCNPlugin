...
- Any content with explicit rank semantics (TOP N, hero rankings, rise/fall boards, or win-rate/pick-rate/Ban-rate rankings) must use one vertical column on both desktop and mobile, with numbers increasing from top to bottom. Do not use two columns, masonry, or horizontal ranking bands. Non-ranking data such as season trends, segment filters, and hero comparisons must not use ranking numbers, TOP labels, or RANKING visual language.
