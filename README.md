...
- 新增 `/每日战绩`，支持今天、昨天、明确日期、UID 和赛季参数，参数顺序不限；`/今日战绩` 是今天的兼容别名。
- 每日战绩使用北京时间半开区间向国服 Summary 接口查询，支持分页、每 10 场一批详情、详情缓存和同 key SingleFlight，不通过最近对局历史扫描兜底。
- 图片展示总计、快速/竞技/其他模式、K/D/A、场均指标和 Top 5 英雄；`gameModeId=4` 在首版保留为其他模式。

## 1.3.0 个人英雄分析系统

- `/我的英雄`、`/我的绝活`、`/我的绝症` 统一使用 `PlayerCareerAnalysisService`，共享同一份完整英雄分析结果。
- 三条命令都支持生涯或指定赛季；UID 与赛季参数顺序不限。分析同时保留快速、竞技的完整模式统计，并使用统一的 -100 到 +100 Performance Index。
...
- 个人英雄数据增加最后一击、场均伤害、场均治疗和场均承伤，保留 `MVP / SVP`；无场次或缺失总量显示 `—`。
- 同一玩家/英雄/最近对局/对局详情请求支持 SingleFlight 合并；外部请求和 HTML→PNG 渲染分别限流，渲染失败最多重试一次并返回短错误提示。
- OneBot 群聊图片结果通过 AstrBot 消息链恢复 @；`/最近对局` 保存 5 分钟数字选择会话，支持回复 `/对局 1` 到 `/对局 10`。QQ Official 原生按钮保持不变。
...
/每日战绩
/每日战绩 昨天
/每日战绩 2026-08-15 195963667 S9.5
/英雄数据 蜘蛛侠 S9.5
/我的英雄 杰夫 S9.5
/英雄环境 大师 S9.5
...
`/每日战绩` 支持 `今天`、`昨天`、`YYYY-MM-DD`、`YYYY/MM/DD`、`MM-DD` 和 `M月D日`；未指定日期时使用今天，未指定 UID 时使用绑定账号，未指定赛季时使用当前默认赛季。未来日期和模糊自然语言日期不会被接受。每日战绩按北京时间自然日查询，空结果会正常显示“当日暂无对局”。
...
`/我的绝活` 和 `/我的绝症` 现在共享 `PlayerCareerAnalysisService` 的完整英雄分析。无赛季时分析全生涯，指定 `S9`、`S9.5` 时只分析该赛季；UID 与赛季参数顺序不限。统一的 `Performance Index` 范围为 -100 到 +100，由同期同段位 Meta、同赛季个人竞技留一基准、同赛季个人快速留一基准按 55% / 30% / 15% 加权，缺失信号会重新归一权重；快速模式永远不直接与 Meta 比较。小样本继续使用收缩修正，Meta 仍按玩家每个赛季历史段位匹配。

绝活指数为 `Play Index × max(Performance Index, 0) ÷ 100`，绝症指数为 `Play Index × max(-Performance Index, 0) ÷ 100`。因此同一英雄不可能同时进入正向绝活和负向绝症。`Play Index` 同时考虑竞技场次、快速场次和英雄使用占比；生涯使用量 cap 为 50 场，单赛季 cap 为 20 场。核心缓存保存全部有记录英雄，单英雄查询不会因为未进入 Top 5/Top 10 而丢失数据。
