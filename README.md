...
群聊中的 Markdown 按钮消息和图片消息都会 @ 本次命令的发起者。`/战绩` 与 `/查询` 的按钮消息不再重复展示战绩文字，只保留 @ 与快捷按钮；完整数据统一放在图片中。指令帮助按“命令、用途”分行并在各命令之间留空行，便于 AstrBot 转图后阅读。

赛季支持 `S0`、`S9`、`S9.5`、`S9上半赛季`、`S9下半赛季`；英雄查询使用中文名称，例如 `/英雄 蜘蛛侠 1287101468 s9上半赛季`。

`/对局` 支持纯 ID、`matchUid=...` 和 `matchUid：...` 三种格式。
