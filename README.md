...
`/我的绝活` 是跨赛季生涯专精分析，只接受可选 UID，不再接受赛季或最低场次参数。系统逐赛季读取玩家竞技数据，并与该赛季玩家历史段位对应的 RivalsMeta 环境比较，再按竞技场次加权汇总；快速模式只参与使用量和本命英雄判定。小样本使用收缩修正，缺失历史 Meta 会降低覆盖率而不会伪造数据；结果不足正式绝活时仍展示最接近的候选。
