...
- 正式入口为 `/我的英雄`，`/英雄数据` 和 `/英雄` 继续兼容；英雄支持常用别称，职责统一为先锋、决斗、战略。
...
已绑定账号后，`UID` 可以省略。职责可写先锋/T/坦克、决斗/C/DPS、战略/奶/辅助；范围可写前 10、Top10、1-10、11-20、最后 10。英雄别称例如 `杰夫`、`鲨鱼` 会归一化为 `陆行鲨杰夫`；裸输入 `死侍` 会提示选择职责版本。
