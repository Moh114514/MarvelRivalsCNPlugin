...
QQ Official 上的 `/战绩`、`/最近`、`/英雄` 和 `/对局` 会输出原生 Markdown + 消息按钮：可从玩家战绩进入最近对局或常用英雄，从最近列表直接打开单局详情。非 QQ Official 平台或富消息发送失败时自动回退原有文本，不会重复请求国服接口。`/卡片测试` 可单独验证账号的 Markdown、指令按钮和链接按钮权限。本插件不依赖 HTML、Playwright 或 Pillow。
