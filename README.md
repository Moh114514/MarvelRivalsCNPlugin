...
- 职责统计 V1 按单局 `matchPlayer.curHeroId` 对应的代表英雄归类；若单局中途跨英雄或跨职责切换，当前接口数据可能产生少量归类偏差，输出不会将其解释为精确的英雄使用时长。
...
`/战绩回顾` 支持 `今天`、`昨天`、`YYYY-MM-DD`、`YYYY/MM/DD`、`MM-DD`、`M月D日`、`今天 14:00-18:00`、`2026-08-15 20:00 2026-08-16 02:00`、`最近6小时` 和 `本周`；未指定时间范围时使用今天，未指定 UID 时使用绑定账号。时间范围按北京时间半开区间查询，未来尚未开始的范围、起点不早于终点的范围和超过 7 天的范围会被拒绝。时间范围查询无需指定赛季。`/每日战绩` 只接受日期和 UID，是整日时间窗口快捷入口；空结果会正常显示“暂无对局记录”。
