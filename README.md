...
`/我的绝症` 是独立的高使用量相对弱势分析，最多展示 Top 10，不再把“绝症”当成严格确诊。候选采用软门槛：总场次至少 10，或竞技至少 5，或快速至少 20，并且至少一个模式有胜率。每个英雄计算“爱玩指数”（竞技 40%、快速 20%、使用占比 40%）和“菜度指数”（同期 Meta 劣势 55%、个人竞技劣势 30%、个人快速劣势 15%）；最终 `绝症指数 = 爱玩指数 × 菜度指数 ÷ 100`。个人劣势使用排除当前英雄的留一法，缺少某类数据时按剩余信号重新分配权重；明显高于同期 Meta 且可比较竞技场次不少于 20 的英雄会被保护，不进入该榜。快速模式是辅助信号，绝症指数只用于相对排序，不代表实际损失。
