...
class QQOfficialCardSender:
    ...
    async def _send_media(
        self, event: Any, card: InteractiveCard, source: Any, bot: Any, api: Any,
        *, image_only: bool = False,
        ...
        media_reply_fields = {"msg_id": reply_id, "msg_seq": 1 if image_only else 2} if reply_id else {}
        markdown_payload = self.build_payload(event, card)
        if reply_id and not image_only:
    ...
    async def send(self, event: Any, card: InteractiveCard, *, image_only: bool = False) -> None:
        ...
        if card.image_url:
            await self._send_media(event, card, source, bot, api, image_only=image_only)
            return
