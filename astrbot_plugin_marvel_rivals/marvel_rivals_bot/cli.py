...
from .services.rivals import RivalsService, format_player
...
async def run(args: argparse.Namespace) -> None:
    ...
    try:
        if args.command == "player":
            print(format_player(await source.get_player(args.uid)))
        elif args.command == "recent":
            matches = await source.get_recent_matches(args.uid)
            print(service and _format_recent(matches))
        elif args.command == "hero":
            print(await service.hero_text(args.uid, args.hero_id))
        elif args.command == "match":
            print(await service.match_detail_text(args.match_uid))
    except DataSourceError as exc:
        raise SystemExit(f"查询失败：{exc}") from exc


def _format_recent(matches: list[dict[str, Any]]) -> str:
    if not matches:
        return "没有返回对局记录。"
    lines = ["最近对局"]
    for item in matches:
        match_uid = item.get("matchUid", item.get("matchUID", item.get("id", "-")))
        lines.append(f"matchUid={match_uid}  {json.dumps(item, ensure_ascii=False, separators=(',', ':'))}")
    return "\n".join(lines)
